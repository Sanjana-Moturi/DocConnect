            ...
            {doctors.slice(0,10).map((items,index)=>(
                <div onClick={()=>navigate(`/appointment/${items._id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
                    ...
                    <div className='p-4'>
                        ...
                        <p className='font-medium text-lg text-gray-900'>{items.name} </p>
                        <p className='text-sm text-gray-600'>{items.speciality}</p>
                    </div>
                </div>
            ))}
        </div>
        <button onClick={()=>{navigate('/doctors'); scrollTo(0,0)}} className='bg-blue-50 text-gray-600 rounded-full px-12 py-3 mt-10 cursor-pointer'>more</button>
